...
export async function GET(){
 ...
 const rows=await sql`WITH segment_data AS (
  ...
  WHERE cl.name='Venluto' ORDER BY c.domain,tc.id`;
 ...
 return new Response(csv,{headers:{"content-type":"text/csv; charset=utf-8","content-disposition":'attachment; filename="TAM-Venluto.csv"',"cache-control":"no-store"}});
}
