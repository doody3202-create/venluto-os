...
type Payload = {
  ...
  preview_text?: string; reply_body?: string; received_at?: string; event_timestamp?: string;
  secret_key?: string; reply_category?: string|number;
...
export async function POST(request:Request){
 ...
 const name=(p.to_name??"").trim().split(/\s+/);const firstName=p.first_name??name.shift()??email.split('@')[0],lastName=p.last_name??name.join(' ');
 ...
 const rawReply=p.reply_message?.text??p.preview_text??p.reply_text??p.message??p.reply_body??p.reply_message?.html??'(No reply body)',replyBody=plain(rawReply)||'(No reply body)';
