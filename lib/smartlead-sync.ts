...
export async function syncVenlutoSmartleadCampaigns(force=false){
 ...
 for(const item of campaigns){
  ...
  try{const statsResponse=await fetch(`https://server.smartlead.ai/api/v1/campaigns/${externalId}/analytics?api_key=${encodeURIComponent(apiKey)}`,{cache:"no-store"});if(statsResponse.ok)stats=unwrap(await statsResponse.json());}catch{}
