...
export async function tamDashboard(clientId?:number){
 await ensureDatabase();
 let client;if(clientId)[client]=await sql`SELECT id,name FROM clients WHERE id=${clientId} AND status='active'`;else [client]=await sql`INSERT INTO clients(name) VALUES ('Venluto') ON CONFLICT(name) DO UPDATE SET status='active' RETURNING id,name`;
