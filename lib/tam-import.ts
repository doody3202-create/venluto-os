...
export async function commitImport(batchKey:string,clientName:string,limit=500){
 ...
 return sql.begin(async tx=>{
  ...
  for(const row of rows){
   ...
   if((email||linkedin)&&!existingContactId)await tx`INSERT INTO tam_contacts(client_id,company_id,source_batch_id,first_name,last_name,title,email,normalized_email,linkedin_url,email_status,eligibility_status,eligibility_reason) VALUES (${batch.client_id},${company.id},${sourceBatch.id},${clean(p.first_name)||'Unknown'},${clean(p.last_name)},${clean(p.title)},${email||null},${email||null},${linkedin||null},${email?'unverified':'missing'},${eligibility},${eligibilityReason})`;
