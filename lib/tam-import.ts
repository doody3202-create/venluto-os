...
export type TamImportRecord={company_name?:string;domain?:string;linkedin_url?:string;provider_id?:string;country?:string;segment?:string;segments?:string[];icp_status?:string;icp_reason?:string;first_name?:string;last_name?:string;title?:string;email?:string;contact_linkedin_url?:string;source_provider?:string;eligibility_status?:string;eligibility_reason?:string;[key:string]:unknown};
...
export async function commitImport(batchKey:string,clientName:string,limit=500){
 ...
 return sql.begin(async tx=>{
  ...
  const [sourceBatch]=await tx`INSERT INTO source_batches(client_id,provider,external_batch_id,label,raw_count,metadata_json) VALUES (${batch.client_id},${batch.source},${batch.external_batch_key},${batch.label},${batch.received_count},${sql.json({tamImportBatchId:batch.id})}) ON CONFLICT(client_id,provider,external_batch_id) DO UPDATE SET raw_count=EXCLUDED.raw_count,label=EXCLUDED.label RETURNING id`;
  ...
  for(const row of rows){
   ...
   const [cc]=await tx`INSERT INTO client_companies(client_id,company_id,source_batch_id,icp_status,icp_reason,attributes_json) VALUES (${batch.client_id},${company.id},${sourceBatch.id},${clean(p.icp_status)||'unreviewed'},${clean(p.icp_reason)||null},${sql.json(attributes)}) ON CONFLICT(client_id,company_id) DO UPDATE SET source_batch_id=COALESCE(client_companies.source_batch_id,EXCLUDED.source_batch_id),icp_status=CASE WHEN EXCLUDED.icp_status<>'unreviewed' THEN EXCLUDED.icp_status ELSE client_companies.icp_status END,icp_reason=COALESCE(EXCLUDED.icp_reason,client_companies.icp_reason),attributes_json=client_companies.attributes_json||EXCLUDED.attributes_json,updated_at=NOW() RETURNING id`;
   ...
   const email=normalizeEmail(clean(p.email)),linkedin=clean(p.contact_linkedin_url),companyDecision=clean(p.icp_status).toLowerCase(),requestedEligibility=clean(p.eligibility_status).toLowerCase(),isCompanyHeld=['hold','held','excluded','rejected','not_fit'].includes(companyDecision),eligibility=isCompanyHeld?'held':requestedEligibility||'eligible',eligibilityReason=isCompanyHeld?`Company ICP status is ${companyDecision}`:clean(p.eligibility_reason)||null;let existingContactId:number|null=null;
   if(email&&linkedin)existingContactId=((await tx`SELECT id FROM tam_contacts WHERE client_id=${batch.client_id} AND (normalized_email=${email} OR linkedin_url=${linkedin}) LIMIT 1`)[0]?.id as number|undefined)??null;
   else if(email)existingContactId=((await tx`SELECT id FROM tam_contacts WHERE client_id=${batch.client_id} AND normalized_email=${email} LIMIT 1`)[0]?.id as number|undefined)??null;
   else if(linkedin)existingContactId=((await tx`SELECT id FROM tam_contacts WHERE client_id=${batch.client_id} AND linkedin_url=${linkedin} LIMIT 1`)[0]?.id as number|undefined)??null;
   if((email||linkedin)&&!existingContactId)await tx`INSERT INTO tam_contacts(client_id,company_id,source_batch_id,source_provider,first_name,last_name,title,email,normalized_email,linkedin_url,email_status,eligibility_status,eligibility_reason) VALUES (${batch.client_id},${company.id},${sourceBatch.id},${clean(p.source_provider)||batch.source},${clean(p.first_name)||'Unknown'},${clean(p.last_name)},${clean(p.title)},${email||null},${email||null},${linkedin||null},${email?'unverified':'missing'},${eligibility},${eligibilityReason})`;
   else if(existingContactId&&(isCompanyHeld||requestedEligibility))await tx`UPDATE tam_contacts SET eligibility_status=${eligibility},eligibility_reason=${eligibilityReason},updated_at=NOW() WHERE id=${existingContactId} AND (eligibility_status='eligible' OR eligibility_reason LIKE 'Company ICP status is %')`;
