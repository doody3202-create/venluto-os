...
export async function commitImport(batchKey:string,clientName:string,limit=500){
 ...
 return sql.begin(async tx=>{
  ...
  for(const row of rows){
   ...
   const email=normalizeEmail(clean(p.email)),linkedin=clean(p.contact_linkedin_url),companyDecision=clean(p.icp_status).toLowerCase(),isHeld=['hold','held','excluded','rejected','not_fit'].includes(companyDecision),eligibility=isHeld?'held':'eligible',eligibilityReason=isHeld?`Company ICP status is ${companyDecision}`:null;let existingContactId:number|null=null;
   ...
   if((email||linkedin)&&!existingContactId)await tx`INSERT INTO tam_contacts(client_id,company_id,first_name,last_name,title,email,normalized_email,linkedin_url,email_status,eligibility_status,eligibility_reason) VALUES (${batch.client_id},${company.id},${clean(p.first_name)||'Unknown'},${clean(p.last_name)},${clean(p.title)},${email||null},${email||null},${linkedin||null},${email?'unverified':'missing'},${eligibility},${eligibilityReason})`;
   else if(existingContactId&&isHeld)await tx`UPDATE tam_contacts SET eligibility_status='held',eligibility_reason=${eligibilityReason},updated_at=NOW() WHERE id=${existingContactId} AND eligibility_status='eligible'`;
